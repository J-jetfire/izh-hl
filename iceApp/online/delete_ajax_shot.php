...
$sql = "SELECT * FROM `$squad` WHERE `id` = '$id'"; // ОПРЕДЕЛЕНИЕ ТЕКУЩЕГО КУБКА И БАЗЫ
    $records = mysqli_query($link,$sql);
        while ($row = mysqli_fetch_array($records)){
            $goal = $row['shot'];
        }
if ($goal > 0) {
    ...
    $sql = "UPDATE `$players` SET `shot` = `shot` - '1' WHERE id = '$id' ";
    $records = mysqli_query($link, $sql);
     // SCORER
    $sql = "UPDATE `$squad` SET `shot` = `shot` - '1' WHERE id = '$id' ";
    $records = mysqli_query($link, $sql);
    // SHOT PERCENTAGE
    $sql = "UPDATE `$players` SET `shotperc`=(`goal`*100/`shot`) WHERE `shot`!='0' AND id = '$id' ";
    $records = mysqli_query($link, $sql);

    // GOALKEEPER - ALL SHOTS
    $sql = "UPDATE `$players` SET `allshot` = `allshot` - '1' WHERE id = '$lost' ";
    $records = mysqli_query($link, $sql);
    $sql = "UPDATE `$squad` SET `allshot` = `allshot` - '1' WHERE id = '$lost' ";
    $records = mysqli_query($link, $sql);
