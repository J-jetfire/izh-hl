...
if ($goal > 0) {
    ...
    $sql = "UPDATE `$players` SET `goal` = `goal`-'1', `shot` = `shot` - '1', `point`=`point` - '1' WHERE `goal`>0 AND `id` = '$id' ";
    $records = mysqli_query($link, $sql);
     // SCORER
    $sql = "UPDATE `$squad` SET `goal` = `goal`-'1', `shot` = `shot` - '1', `point`=`point` - '1' WHERE `goal`>0 AND id = '$id' ";
    $records = mysqli_query($link, $sql);
